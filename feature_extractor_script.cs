...
public class TrancoDataRow
{
    ...
    public int Label { get; set; } // Will be read from the input file
...
public class Program
{
    ...
    // Make sure to update these paths if needed
    private const string TRONCO_FILE = @"C:\Users\Mihir Maurya\Downloads\merged_dataset.csv"; // Your input file
    private const string OUTPUT_FULL = @"C:\Users\Mihir Maurya\Downloads\rebuilt_dataset_full.csv"; // Your output file

    // WARNING: 2000 is a very high number. This may cause network errors or get your IP banned.
    // Recommended to start with 200-500 and increase if stable.
    private const int HTTP_CONNECTIONS = 2000;
    ...
    private static async Task<HeadResult> HeadWorkerAsync(string url)
    {
        ...
        var normalizedUrl = (url.Contains("://")) ? url : $"http://{url}";
    ...
    public static async Task Main(string[] args)
    {
        ...
        // --- MODIFICATION 1 ---
        // 1. Read both URL (col 0) and Label (col 1) from the file
        var inputData = File.ReadLines(TRONCO_FILE)
                           .AsParallel() // Parallelize reading the file
                           .Select(line =>
                           {
                               var parts = line.Split(',');
                               int label = 0; // Default to 0 if parsing fails
                               if (parts.Length > 1)
                               {
                                   int.TryParse(parts[1], out label);
                               }
                               // Assumes URL is in the first column (index 0)
                               return new { Url = parts[0], Label = label };
                           })
                           .ToList();

        // 2. Extract just the URLs for the network fetch
        var urls = inputData.Select(d => d.Url).ToList();
        ...
        // --- MODIFICATION 2 ---
        // Combine the original input (with Label) with the network results
        var dataToProcess = inputData.Zip(headFeatures, (data, head) => new { data.Url, data.Label, Head = head });
        ...
        var finalRows = dataToProcess
            ...
            {
                // --- MODIFICATION 3 ---
                // Assign the Label from the input file
                var row = new TrancoDataRow { url = item.Url, Label = item.Label };

                #region Deconstruct URL and Calculate All Lexical Features
                string domain = "", directory = "", file = "", query = "", fragment = "";
                try
                {
                    // Ensure URL has a scheme for Uri parser to work correctly
                    var normalizedUrl = (item.Url.Contains("://")) ? item.Url : $"http://{item.Url}";
